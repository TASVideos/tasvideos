...
internal class ExternalMediaPublisher(AppSettings appSettings, IEnumerable<IPostDistributor> providers, IHttpContextAccessor httpContextAccessor) : IExternalMediaPublisher
{
	...
	public string ToAbsolute(string relativeLink)
		=> !string.IsNullOrWhiteSpace(relativeLink)
...
{
	extension(IExternalMediaPublisher publisher)
	{
		public async Task SendMessage(string group, string message, string body = "")
		...
		public async Task SendAdminMessage(string group, string message)
			=> await publisher.Send(new Post
			{
		...
		public async Task SendUserFile(bool unlisted, string formattedTitle, long id, string fileTitle)
		{
			var unformattedTitle = Unformat(formattedTitle);
			await publisher.Send(new Post
			{
				Type = unlisted
		...
		public async Task AnnounceNewSubmission(int subId, string subTitle, byte[]? imageData = null, string? imageMimeType = null, int? imageWidth = null, int? imageHeight = null)
			=> await publisher.Send(new Post
			{
		...
		public async Task SendSubmissionEdit(int subId, string formattedTitle, string body, bool force = false)
		...
		public async Task SendDeprecation(string formattedTitle)
			=> await publisher.Send(new Post
			{
				Group = PostGroups.Submission,
				Title = Unformat(formattedTitle),
		...
		public async Task AnnounceNewPublication(int publicationId, string publicationTitle, byte[]? imageData = null, string? imageMimeType = null, int? imageWidth = null, int? imageHeight = null)
			=> await publisher.Send(new Post
			{
		...
		public async Task SendUnpublish(string publicationTitle, int id, string reason)
			=> await publisher.Send(new Post
			{
		...
		public async Task SendSubmissionDelete(string submissionTitle, int id)
			=> await publisher.Send(new Post
			{
				Type = PostType.General,
				Group = PostGroups.Submission,
		...
		public async Task SendPublicationEdit(string userName, int publicationId, string body)
		{
		...
		public async Task SendPublicationClassChange(int id, string pubTitle, string userName, string oldClass, string newClass)
			=> await publisher.Send(new Post
			{
				Group = PostGroups.Publication,
		...
		public async Task AnnounceNewsPost(string formattedTitle, string body, int postId)
			=> await publisher.Send(new Post
			{
		...
		public async Task SendForum(bool restricted, string formattedTitle, string body, string relativeLink)
			=> await publisher.Send(new Post
			{
				Type = restricted
					? PostType.Administrative
		...
		public async Task SendWiki(string formattedTitle, string body, string path, bool force = false)
			=> await publisher.Send(
				new Post
				{
		...
		public async Task SendUserManagement(string formattedTitle, string userName)
			=> await publisher.Send(new Post
			{
				Type = PostType.Administrative,
		...
		public async Task SendRoleManagement(string formattedTitle, string roleName)
			=> await publisher.Send(new Post
			{
				Type = PostType.Administrative,
				Group = PostGroups.UserManagement,
		...
		public async Task SendGameManagement(string formattedTitle, string body, string relativeLink)
			=> await publisher.Send(new Post
			{

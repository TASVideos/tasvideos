...
@echo This does not automate the following: Exotic, TAS of, TASer of, Rookie of.
...
FOR %%a in (*.png) DO ffmpeg -n -hide_banner -i "%%~a" -i T2024.png -filter_complex "[0:v]crop=iw:ih-15:0:0[main]; [main][1:v]vstack" "output/%%~a"
FOR %%a in (*.png) DO ffmpeg -n -hide_banner -i "%%~a" -i P2024.png -filter_complex "[0:v]crop=iw:ih-17:0:0[main]; [main][1:v]vstack" "output/%%~a"
	...
	public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
	{
		...
		output.Content.AppendHtml($"""
									<div class='p-2 text-center d-none border border-primary rounded' id='show-{context.UniqueId}'>
										<a href='#' onclick='return false;'><h4 class='m-0'><i class='fa fa-chevron-{(Reverse ? "up" : "down")}'></i> {ShowText}</h4></a>
									</div>
									""");
		output.Content.AppendHtml($"""
									<div class='p-2 text-center d-none border border-primary rounded' id='hide-{context.UniqueId}'>
										<a href='#' onclick='return false;'><h4 class='m-0'><i class='fa fa-chevron-{(Reverse ? "down" : "up")}'></i> {HideText}</h4></a>
									</div>
									""");
		output.Content.AppendHtml($$"""
									<script>

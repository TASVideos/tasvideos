...
public class Fm2ParserTests : BaseParserTests
{
	...
	{
		var result = await _fm2Parser.Parse(Embedded("norerecords.fm2", out var length), length);
	...
	{
		var result = await _fm2Parser.Parse(Embedded("negativererecords.fm2", out var length), length);
		Assert.IsTrue(result.Success);
	...
	{
		var result = await _fm2Parser.Parse(Embedded("binary.fm2", out var length), length);
		Assert.IsTrue(result.Success, "Result is successful");
		Assert.AreEqual(2, result.Frames, "Frame count should be 2");
		AssertNoWarningsOrErrors(result);
	}
	...
	{
		var result = await _fm2Parser.Parse(Embedded("binarynolength.fm2", out var length), length);
	...
	{
		var result = await _fm2Parser.Parse(Embedded("hash-invalid.fm2", out var length), length);
	...
	{
		var result = await _fm2Parser.Parse(Embedded("hash-missing.fm2", out var length), length);
		Assert.AreEqual(0, result.Hashes.Count);
	}
}

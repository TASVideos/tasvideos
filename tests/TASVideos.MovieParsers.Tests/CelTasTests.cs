...
public class CelTasTests : BaseParserTests
{
	...
	{
		var result = await _celtasParser.Parse(Embedded(filename), EmbeddedLength(filename));
	...
	{
		var result = await _celtasParser.Parse(Embedded("2465ms.celtas"), EmbeddedLength("2465ms.celtas"));

...
public static class Paginator
{
		...
		where TItem : class
	{
		...
		public async Task<PageOf<TItem>> SortedPageOf(PagingModel paging)
			=> await query
				.SortBy(paging)
				.PageOf(paging);

		public async Task<PageOf<TItem, TRequest>> SortedPageOf<TRequest>(TRequest paging)
			where TRequest : PagingModel
			=> await query
				.SortBy(paging)
				.PageOf(paging);

		public async Task<PageOf<TItem>> PageOf(PagingModel paging)
		{
			var rowsToSkip = paging.Offset();
			...
			{
				var pageSize = Math.Max(paging.PageSize.Value, 1);
		...
		public async Task<PageOf<TItem, TRequest>> PageOf<TRequest>(TRequest paging)
			where TRequest : PagingModel
		{
			var rowsToSkip = paging.Offset();
	...
	{
		var desc = column?.StartsWith('-') ?? false;
		...
		sortApplied = true;
		return SortByParamInner(query, column, desc: desc, thenBy: thenBy);
	}

	private static IQueryable<T> SortByParamInner<T>(IQueryable<T> query, string column, bool desc, bool thenBy)
	{
		string orderBy;
		if (thenBy)
		{
